...
# Usage: ./agent_run_local.sh [same args as script.sh, e.g. --testnet]
...
echo "  After setup, use ./agent_headless.sh for CLI-only operations (setup, signin, create-payment-link)."
echo "  If you just ran ./agent_headless.sh reset-local, choose option 1 (Install PayRam) to deploy a new container."
echo ""

exec sudo -E ./agent_script.sh "$@"
