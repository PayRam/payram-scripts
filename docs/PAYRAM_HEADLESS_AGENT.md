...
- PayRam must be running (e.g. `./run_local.sh` → choose Install or Restart).
...
Run from repo root: `./headless.sh [command]`
...
| `reset-local [-y]` | Wipe local DB and API data; then run `./run_local.sh` and choose Install PayRam |
...
1. **Start PayRam:** `./run_local.sh` → option 1 (Install) or restart.
2. **Auth:** `./headless.sh signin` (or setup if first time). Env: `PAYRAM_EMAIL`, `PAYRAM_PASSWORD`.
3. **Config (local):** `./headless.sh ensure-config` so payment creation works.
4. **Wallet:** Either `./headless.sh ensure-wallet` (BTC) or `./headless.sh deploy-scw` (ETH SCW). deploy-scw needs Sepolia ETH on deployer address for gas (default RPC and fund collector).
5. **Payment link:** `./headless.sh create-payment-link` or pass `[projectId] [email] [amountUSD]`. Use the printed URL as-is (keep `&host=...`).
...
- `./headless.sh reset-local [-y]` clears DB and API data (and optionally Docker image).
- Then run `./run_local.sh` and choose **1) Install PayRam** (not just Restart).
...
- **Scripts:** `scripts/generate-deposit-wallet.js` (BTC), `scripts/generate-deposit-wallet-eth.js` (ETH xpub), `scripts/deploy-scw-eth.js` (SCW deploy). Run via headless commands; deploy-scw is invoked by `./headless.sh deploy-scw`.
...
| Issue | Action |
|-------|--------|
| API unreachable | Ensure PayRam is running (`./run_local.sh`). Check `PAYRAM_API_URL`. |
| Auth expired / 401 | Run `./headless.sh signin` again. |
