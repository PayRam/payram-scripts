...
# Requires PayRam already running (e.g. via ./agent_run_local.sh).
# Usage: ./agent_headless.sh [command] [options]
...
#   reset-local        - Remove local data; run ./agent_run_local.sh again to start fresh
...
cmd_status() {
  ...
  if [[ "$res" == "200" ]]; then
    ...
    echo "API: unreachable (is PayRam running? try ./agent_run_local.sh)"
    return 1
  fi
  load_tokens
  if [[ -n "${ACCESS_TOKEN:-}" ]]; then
...
cmd_reset_local() {
  ...
  echo "Then you can run ./agent_run_local.sh again and use ./agent_headless.sh (setup, wallet, payment link)."
  echo ""
  ...
  echo "Database and API data cleared. Run ./agent_run_local.sh and choose option 1 (Install PayRam) to start fresh."
  echo ""
...
cmd_run() {
  ...
  code=$(echo "$res" | tail -1)
  if [[ "$code" != "200" ]]; then
    echo "API unreachable. Is PayRam running? (e.g. ./agent_run_local.sh)"
    return 1
  fi
